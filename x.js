...
export default class RESTResource {
  ...
  reducer(state = [], action) {
    ...
    if (dataKey !== this.dataKey) {
      console.log(`ignoring ${action.type} action with dataKey ${dataKey} != ${this.dataKey}`);
      return state;
    }
    ...
    switch (action.type) {
      ...
      default: {
        if (dataKey === this.dataKey) {
          console.log(`> RESTResource reducer handling '${action.type}' with dataKey=${dataKey}`);
          if (action.type.endsWith('FETCH_SUCCESS')) console.log(' FETCH_SUCCESS:', action);
          return this.crudReducers(state, action);
        } else {
          console.log(`< RESTResource reducer ignoring '${action.type}' with dataKey=${dataKey} (not ${this.dataKey})`);
          return state;
        }
      }
    }
  ...
  fetchAction = (props) => {
    ...
    return (dispatch, getState) => {
      ...
      console.log(`*** fetchStart (dataKey '${this.dataKey}') for ${url}`);
      dispatch(crudActions.fetchStart(url));
      return fetch(url, { headers })
        .then((response) => {
          if (response.status >= 400) {
            response.text().then((text) => {
              error(dispatch, 'GET', crudActions.fetchError(url), null, this.module, this.name,
                    { status: response.status, message: text });
            });
          } else {
            response.json().then((json) => {
              ...
              if (!data) {
                error(dispatch, 'GET', crudActions.fetchError(url), null, this.module, this.name, `no records in '${records}' element`);
                return;
              }
